    ...
    async def load_pdf(self, file_path: str) -> List[Document]:
        ...
        """
         try:
             ...
             logging.info(f"Loading PDF: {file_path} ({file_size / 1024 / 1024:.2f} MB)")
    ...
    async def process_pdf(self, file_path: str) -> Tuple[List[Document], int]:
        ...
        try:
            ...
            logger.info(
                ...
                f"{len(chunks)} chunks, ~{estimated_tokens} tokens"
            )
...
def get_processor(chunk_size: int = 1000, chunk_overlap: int = 20) -> PDFProcessor:
